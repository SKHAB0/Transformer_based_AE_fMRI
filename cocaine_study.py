...
def train_model(embed_dim=embed_dim, 
    ...
    new_path = 'trained_models_cocaine/' + model_type + '/' + position + '/' + layers
...
#Comparing models 
# train_model(model_type = 'Decomposition_Transformer')
# train_model(model_type = 'Standard_Transformer')
# train_model(model_type = 'Autocorr_Transformer')


model_autocorr = torch.load('trained_models_cocaine/Autocorr_Transformer/latent/32_16_4_2/trained_model_2heads_FE2_lambda-0.1_physical_Autocorr_Transformer.pth')
model_decomposition = torch.load('trained_models_cocaine/Decomposition_Transformer/latent/32_16_4_2/trained_model_2heads_FE2_lambda-0.1_physical_Decomposition_Transformer.pth')
model_standard = torch.load('trained_models_cocaine/Standard_Transformer/latent/32_16_4_2/trained_model_2heads_FE2_lambda-0.1_physical_Standard_Transformer.pth')
...
# plt.tight_layout()
# plt.savefig('figures/cocaine/losses.png')
...
# concatenated_train = np.concatenate(encoded_train, axis=0)
# sequence_lengths = [train_data.shape[1]] * train_data.shape[0]

# n_components = 2 
# n_iter = 10000

# model = hmm.GaussianHMM(n_components=n_components, covariance_type="diag", n_iter=n_iter)
# model.fit(concatenated_train, lengths=sequence_lengths)
...
encoded_val = model_autocorr['model'].encode(val_loader.dataset.tensors[0], None).detach().numpy()
labels_val = val_loader.dataset.tensors[1].detach().numpy()

encoded_test = model_autocorr['model'].encode(test_loader.dataset.tensors[0], None).detach().numpy()
